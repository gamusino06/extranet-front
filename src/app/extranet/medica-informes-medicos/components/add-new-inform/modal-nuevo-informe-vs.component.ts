...
@Component({
  selector: 'app-modal-nuevo-informe-vs',
  templateUrl: './modal-nuevo-informe-vs.component.html',
  styleUrls: ['./modal-nuevo-informe-vs.component.css']
})
export class ModalNuevoInformeVsComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<ModalNuevoInformeVsComponent>,
  ...
  setInitDoc() {
    this.modalNuevoInformeForm.controls.tipoDocForm.setValue(118);
  }
  setInitDates() {
    const now = new Date();
  ...
  getTipoDocumento() {
    this.userService.getSubtiposDocumento([this.globals.informes_medicos]).subscribe(result => {
      if (result) {
        this.tipoDocumentoList = result.filter(val => val.idTipoDocumento.toString() === this.globals.documentacion_propia_vs);
  ...
  getSubTipoDocumento() {
    this.userService.getSubtiposDocumento([this.globals.documentacion_propia_vs]).subscribe(result => {
      if (result) {
