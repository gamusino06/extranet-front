...
@Component({
  selector: "app-select-centro-filter-v2-ppt",
  templateUrl: "./select-centro-filter-v2-ppt.component.html",
  styleUrls: ["./select-centro-filter-v2-ppt.component.scss"],
})
export class SelectCentroFilterV2PptComponent implements OnInit {
  ...
  public filteredCentrosMulti: ReplaySubject<Centro[]> = new ReplaySubject<
    Centro[]
  >(1);
  // tslint:disable-next-line:variable-name
  protected _onDestroy = new Subject<void>();

  constructor(public utils: UtilsService, private globals: Globals) {}
  ...
  protected filterCentrosMulti() {
    ...
    this.filteredCentrosMulti.next(
      this.empresasList.filter(
        (empresa) => empresa?.nombre?.toLowerCase().indexOf(search) > -1
      )
    );
  }
    ...
    if (this.empresasList !== undefined) {
      ...
      this.empresasList.forEach((empresa) => {
        empresa.centros.forEach((centro) => {
          this.utils.actualizacionAtributosSegunAreaisPTisVS(
            centro,
            isPT,
            isVS
          );
