...
export class NewPasswordComponent implements OnInit {
  ...
  newPassword() {
    this.userService.newPassword(this.tokenRecibido).subscribe(data => {
      this.tokenRecibido = data;
      if (data) {
        this.user = data.user;
        this.avisoPattern =  data.texto;
        this.regrexPattern= data.patron;
        this.regrexPattern=this.regrexPattern.substring(0,this.regrexPattern.length-1);
        this.changePassForm.controls.password.setValidators([Validators.required, Validators.pattern(this.regrexPattern)]);
        data.texto=this.avisoPattern;
        /*let aux=document.getElementById('patron')
        aux.innerHTML=data.texto;*/
      } else {
        this.router.navigate([this.globals.login]);

        let titulo = "Enlance no válido o caducado";
        this.translate.get('ENLACE_NO_VALIDO_O_CADUCADO').subscribe((res: string) => {
          titulo = res;
        });
